...
def run_predict(predictor, *, context_features, x_timestamp, future_timestamp, pred_len):
    with torch.no_grad():
...
def expected_output_map():
    return {
        context_len: pd.read_csv(
            TEST_DATA_ROOT / f"regression_output_{context_len}.csv",
            parse_dates=["timestamps"],
        )
        for context_len in TEST_CTX_LEN
    }
...
def test_kronos_predictor_regression(context_len, regression_input_df, expected_output_map, predictor):
    expected_df = expected_output_map[context_len]

    if regression_input_df.shape[0] < context_len + len(expected_df):
        raise ValueError("Example data does not contain enough rows for the regression test.")

    context_df = regression_input_df.iloc[:context_len].copy()
    context_features = context_df[FEATURE_NAMES].reset_index(drop=True)
    x_timestamp = context_df["timestamps"].reset_index(drop=True)
    future_timestamp = regression_input_df["timestamps"].iloc[
        context_len : context_len + len(expected_df)
    ].reset_index(drop=True)
    expected = expected_df[FEATURE_NAMES].to_numpy(dtype=np.float32)
...
def test_kronos_predictor_mse(context_len, expected_mse, regression_input_df, predictor):
    if regression_input_df.shape[0] <= context_len + MSE_PRED_LEN:
        raise ValueError("Example data does not contain enough rows for the random sample regression test.")

    valid_region = regression_input_df.iloc[context_len : regression_input_df.shape[0] - MSE_PRED_LEN]
    ...
    mse_values = []
    for row_idx in tqdm(sampled_rows.index.to_list()):
        context_slice = regression_input_df.iloc[row_idx - context_len : row_idx].copy()
        future_slice = regression_input_df.iloc[row_idx : row_idx + MSE_PRED_LEN].copy()

        pred_df = run_predict(
            predictor,
            context_features=context_slice[FEATURE_NAMES].reset_index(drop=True),
            x_timestamp=context_slice["timestamps"].reset_index(drop=True),
            future_timestamp=future_slice["timestamps"].reset_index(drop=True),
